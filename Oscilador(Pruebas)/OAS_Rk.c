...
#define Num_t (float)(1000)//Tiempo final
#define dt (float)(0.1)//Paso en tiempo
#define Num_T (float)(20)//Temperatura final
...
int main()
{
    ...
    for(int k=0;k<trayec;k++)for(int t=0;t<Num_t;t++)zz[k][t]=box_muller(0,1);
    ...
    for(float T=0;T<=dT*Num_T;T+=dT)/*Para varias temperaturas*/
    {
        ...
        fprintf(D1,"%f\t %f\t %f\n",T,0.5*M*D[0]/(float)(trayec),0.5*M*D[1]/(float)(trayec)/2/(dt*Num_t));
    }
